export const faqResponses = [
  {
    id: 'services',
    keywords: ['services', 'offer', 'provide', 'what do you do', 'care'],
    question: 'What services does Althea-Lapuz Lying-In Clinic offer?',
    answer: 'We offer comprehensive maternal and newborn care, including prenatal checkups, normal spontaneous delivery (NSD), newborn screening, vaccination, and family planning services.'
  },
  {
    id: 'book',
    keywords: ['appointment', 'book', 'schedule', 'reserve', 'set up'],
    question: 'How do I book an appointment?',
    answer: 'You can book an appointment by contacting us via phone, email, or our Facebook page. We recommend scheduling in advance for regular checkups, though walk-ins are accepted for urgent maternity needs.'
  },
  {
    id: 'walkin',
    keywords: ['walk in', 'walk-in', 'need to schedule', 'without appointment'],
    question: 'Do I need to schedule an appointment before visiting?',
    answer: 'While we encourage scheduling appointments for prenatal checkups to minimize wait times, our delivery and birthing services are available 24/7 for expecting mothers.'
  },
  {
    id: 'bring',
    keywords: ['bring', 'prepare', 'what to bring', 'documents', 'clothes'],
    question: 'What should I bring for my appointment or delivery?',
    answer: 'For a prenatal checkup, please bring your previous medical records and ultrasound results. For delivery, please prepare your maternity records, baby clothes, receiving blankets, and your personal hygiene kit.'
  },
  {
    id: 'maternity',
    keywords: ['maternity', 'prenatal', 'ultrasound', 'pregnancy'],
    question: 'What maternity and prenatal care services are available?',
    answer: 'Our prenatal care includes routine fetal monitoring, maternal health assessments, nutritional guidance, and formal medical requests for required laboratory tests and ultrasound. Note: In-clinic lab tests and ultrasound are not performed directly; we provide official requests and interpret your results.'
  },
  {
    id: 'postpartum',
    keywords: ['postpartum', 'after birth', 'breastfeeding', 'recovery'],
    question: 'Do you provide postpartum care?',
    answer: 'Yes, we provide postpartum support to monitor the mother\'s recovery, assist with breastfeeding, and ensure the newborn is healthy and thriving.'
  },
  {
    id: 'contact',
    keywords: ['contact', 'phone', 'email', 'call', 'reach', 'number'],
    question: 'How can I contact the clinic?',
    answer: 'You can reach us by phone at 0931 069 3921 / (044)-462-0789, email us at lapuzaltheajasmine@gmail.com, message our Facebook page, or visit our clinic at 332 Daan Ramon Magsaysay, Tilapayong, Baliwag, Bulacan.'
  },
  {
    id: 'hours',
    keywords: ['hours', 'time', 'open', 'close', 'when', 'schedule'],
    question: 'What are the clinic\'s operating hours?',
    answer: 'Our regular checkup hours are Monday to Saturday from 8:00 AM to 5:00 PM. However, our delivery room and on-call birthing team are available 24/7 for any spontaneous deliveries.'
  }
];

export const fallbackResponse = "Sorry, I can currently help with questions about our clinic services, appointments, clinic hours, and other frequently asked questions. Please choose one of the suggested questions below or contact the clinic directly for more assistance.";

export const quickReplyIds = ['book', 'services', 'hours', 'bring', 'contact'];
